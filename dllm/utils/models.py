...
try:
    ...
    _HAS_DIFFUSION_GEMMA = True
except ImportError:
    _HAS_DIFFUSION_GEMMA = False
...
def get_model(
    ...
    **kwargs,
    ...
    if "diffusiongemma" in model_name_or_path.lower():
        # Special case for DiffusionGemma model
        if not _HAS_DIFFUSION_GEMMA:
            raise ImportError(
                "DiffusionGemmaForBlockDiffusion not found. "
                "Upgrade transformers: pip install git+https://github.com/huggingface/transformers.git"
            )
...
def get_tokenizer(
    ...
    # ---------------- Tokenizer loading ----------------
    # Special case for DiffusionGemma: use AutoProcessor
    if "diffusiongemma" in model_name_or_path.lower():
        if not _HAS_DIFFUSION_GEMMA:
            raise ImportError(
                "AutoProcessor for DiffusionGemma not found. "
                "Upgrade transformers: pip install git+https://github.com/huggingface/transformers.git"
            )
