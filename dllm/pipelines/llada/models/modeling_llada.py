...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    def __init__(self, config: LLaDAConfig, model: Optional[LLaDAModel] = None, init_params: bool = False):
        ...
        # Required by transformers v5: `post_init` registers `all_tied_weights_keys`
        # (needed by `infer_auto_device_map`) and other static model properties.
        self.post_init()
    ...
    def tie_weights(self, *args, **kwargs):
        # transformers v5 calls `tie_weights(recompute_mapping=False)` from
        # `post_init`; accept and ignore extra arguments.
        if self.config.weight_tying:
