...
@dataclass
class DreamSamplerWithCompleteHistory(BaseSampler):
    """
    Dream sampler variant that records full history at each diffusion step,
    following the same pattern as MDLMSamplerWithCompleteHistory but using
    Dream-specific decoding (left-padding, right_shift_logits, alg-based
    confidence scoring, alg_temp for position selection, top_p/top_k, CFG
    with pos_id).
    """
    ...
    def sample(
        ...
        **kwargs,
        ...
        for j, L in enumerate(seq_lens):
            if L > 0:
                attention_mask[j, -L:] = 1  # left-padding
        ...
        # For CFG: unconditional input masks out only the prompt
        prompt_index = attention_mask.bool() & (
            torch.arange(T, device=x.device).unsqueeze(0) < T - max_new_tokens
        )
        ...
        for i in range(effective_steps):
            ...
            # --- Confidence scoring via alg ---
            if alg == "maskgit_plus":
                confidence, x0_masked = sample_tokens(
                    mask_logits, temperature=temperature, top_p=top_p, top_k=top_k
                )
            elif alg == "topk_margin":
                confidence, x0_masked = sample_tokens(
            ...
            # Build full x0 prediction tensor
            x0 = x.clone()
            ...
            # Confidence of predicted token at each position
            x0_p = torch.squeeze(
            ...
            # Unmask logprobs: probability of the *current* (already revealed) tokens
            target_ids = x[~mask_index]
            unmask_logits_distribution = p[~mask_index]
            specific_logits = torch.gather(
                unmask_logits_distribution, dim=-1, index=target_ids.unsqueeze(-1)
            ).squeeze(-1)
            full_logprobs_vector = torch.zeros_like(x, dtype=specific_logits.dtype)
            full_logprobs_vector[~mask_index] = specific_logits
            del unmask_logits_distribution, p

            if histories_unmask_logprobs is not None:
                histories_unmask_logprobs.append(full_logprobs_vector.clone())

            H = x0_p - full_logprobs_vector
            if histories_H is not None:
                histories_H.append(H.clone())

            # --- Position selection via confidence + alg_temp ---
            full_confidence = torch.full_like(
                x, -torch.inf, device=self.model.device, dtype=logits.dtype
            )
            full_confidence[mask_index] = confidence
        ...
        if not return_dict:
            ...
            return BaseSamplerOutputCompleteHistory(
                sequences=x,
                ...
                start_idx_history=[T - max_new_tokens] * B,
    ...
    def infill(
        ...
        **kwargs,
        ...
        """
        Fill in-place the tokenizer's `<mask>` tokens contained in `inputs`,
        recording full history at each step.
        """
        # ----- pull args from config, allow kwargs to override -----
        steps = kwargs.get("steps", config.steps)
        eps = kwargs.get("eps", config.eps)
        ...
        # Build right-aligned canvas; left side padded with EOS
        x = torch.full((B, T), eos_token_id, dtype=torch.long, device=self.model.device)
        for i, t in enumerate(inputs):
            L = seq_lens[i]
            x[i, -L:] = t
        ...
        # Precompute per-sample transfer schedule
        mask_index = x == mask_token_id
        num_transfer_tokens_list = get_num_transfer_tokens(
            mask_index=mask_index,
            steps=steps,
        ...
        if not return_dict:
            ...
            return BaseSamplerOutputCompleteHistory(
                ...
                histories_entropy=histories_entropy,
                histories_H=histories_H,
                start_idx_history=[T - sl for sl in seq_lens],
                max_new_tokens=None,
                attention_mask=attention_mask,
            )
